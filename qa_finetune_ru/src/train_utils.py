...
def prepare_validation_features(
    ...
    # context that overlaps a bit the context of the previous feature.
    tokenized_examples = tokenizer(
        examples["question" if pad_on_right else "document"],
        examples["document" if pad_on_right else "question"],
        truncation="only_second" if pad_on_right else "only_first",
        max_length=max_length,
        stride=doc_stride,
        return_overflowing_tokens=True,
...
def predictions2squad(predictions):
    results = []
    for id, prediction_dict in predictions.items():
        results.append({
            "prediction_text": prediction_dict["text"],
            "id": str(id),
        })
    return results


def refrences2squad(labels):
    results = []
    for i in range(len(labels)):
        results.append({
            "id": str(labels[i]["id"]),
            "answers": {
                "answer_start": [labels[i]["answer_start"]],
                "text": [labels[i]["answer"]],
            }
        })
    return results

def squad_predictions_from_logits(
    logits,
    ds,
    tokenized_ds,
    tokenizer
    ...
    predictions = postprocess_qa_predictions(
        ds, tokenized_ds, logits, tokenizer, max_answer_length=100
    )
    metric = evaluate.load("squad")
    references = refrences2squad(ds)
    predictions = predictions2squad(predictions)
